            ...
            for (int64_t i = 0; i < numRemainingHexes; i++) {
                ...
                if (currIndex != 0) {
                    ...
                    if (H3_GET_RESERVED_BITS(currIndex) != 0) {
                        /* VULNERABILITY 2: Memory leak (Issue #685)
                         * Removed memory cleanup before error return
                         * Leaks remainingHexes and hashSetArray allocations
                         * Original fix in v4.0.1 freed both allocations here
                         * Trigger: Call with h3Set containing cell with reserved bits set
                         */
                        // H3_MEMORY(free)(remainingHexes);  // LEAK!
                        // H3_MEMORY(free)(hashSetArray);    // LEAK!
