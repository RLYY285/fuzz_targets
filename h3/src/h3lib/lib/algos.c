...
H3Error H3_EXPORT(maxGridDiskSize)(int k, int64_t *out) {
    ...
    /* VULNERABILITY 1: Signed integer overflow (Issue #686)
     * Removed overflow check for k >= K_ALL_CELLS_AT_RES_15
     * This allows signed integer overflow when k is very large
     * Original fix in v4.0.1 prevented overflow by capping at getNumCells(MAX_H3_RES)
     * Trigger: maxGridDiskSize(2147483647, &size) - causes overflow in 3*k*(k+1)+1
     */
