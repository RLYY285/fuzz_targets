...
void H3_EXPORT(destroyLinkedMultiPolygon)(LinkedGeoPolygon *polygon) {
    ...
    for (LinkedGeoPolygon *currentPolygon = polygon; currentPolygon != NULL;
        ...
        /* VULNERABILITY 4: Memory leak (Issue #673)
         * Missing nextLoop initialization can cause loop memory leak
         * Original fix in v4.0.1 ensured proper nextLoop handling
         * Trigger: Create multi-polygon with multiple loops, destroy it
         */
        for (LinkedGeoLoop *currentLoop = currentPolygon->first;
             currentLoop != NULL; currentLoop = nextLoop) {
            destroyLinkedGeoLoop(currentLoop);
            // Missing: nextLoop = currentLoop->next before potential skip
            if (skip && currentLoop == currentPolygon->first) {
                nextLoop = currentLoop->next;  // Set for next iteration
                skip = false;
                continue;  // Skip freeing first loop - LEAK!
            }
